    ...
    public static class FuncionesExcel
    {
        ...
        public static XLWorkbook ExportaPackingList(List<EncPackingListDto> data, List<DetailPackingListDto> dataDetail, string shipmentCode)
        {
            ...
            foreach (var item in data)
            {
                ...
                hoja.Cell("E" + index).Value = item.bultoEnvio;
                hoja.Cell("F" + index).Value = item.pesoBulto;
                hoja.Cell("G" + index).Value = item.cajaEnvio;
                hoja.Cell("H" + index).Value = item.pesoNeto;
                hoja.Cell("I" + index).Value = item.pesoBruto;
                hoja.Cell("J" + index).Value = item.country;
                hoja.Cell("K" + index).Value = item.state;
                hoja.Cell("L" + index).Value = item.city;
                hoja.Cell("M" + index).Value = item.address;
                hoja.Cell("N" + index).Value = item.zipCode;
                hoja.Cell("O" + index).Value = item.contact;
                hoja.Cell("P" + index).Value = item.email;
                hoja.Cell("Q" + index).Value = item.phone;
                hoja.Cell("R" + index).Value = item.client;
                index++;
            }
